...
const getBankCodes = async function ({ accessToken, bankName }) {
  try {
    const bankCodes = await axios.get(
...
const calculateHash = async function ({
  ...
  try {
    ...
    let data = hash.update(
      `${process.env.MONNIFY_CLIENT_SECRET_KEY}|${paymentReference}|${amountPaid}|${paidOn}|${transactionReference}`,
      "utf-8"
    );
